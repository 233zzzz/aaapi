...
#nodes1 = []
links1 = []    #股东边 up 不必区分入参是holdename还是holderid 因为不像节点需要区分层数
...
nodes4 = []   #持股节点 down 入参为holdername
#links4 = []
#bbb = 0
#print(queryholders(['招商银行股份有限公司'],3))  输入的是一个数组
...
def holder_search():
    try:
        ...
        #print(type(name))
        if direction == "up":
            a1 = queryholders(name, depth)
            b1 = queryholderss(name, depth)
            if len(b1) == 0:  # len(a)不行？？？
                return dumps({"statuscode": 404, "statusmsg": "query no result!"})
            else:
                return dumps({"statuscode": 0, "statusmsg": "success", "result": a1, "links": b1}, ensure_ascii=False)
        elif direction == "down":
            a2 = queryholders2(name, depth)
            b2 = queryholderss2(name, depth)
            if len(b2) == 0:  # len(a)不行？？？
                return dumps({"statuscode": 404, "statusmsg": "query no result!"})
            else:
                return dumps({"statuscode": 0, "statusmsg": "success", "result": a2, "links": b2}, ensure_ascii=False)
        elif direction == "both":
...
def holder_search2():
     ...
     try:
         ...
         #print(type(name))
         if direction == "up":
             a1 = queryholders5(name, depth)
             b1 = queryholderss(name, depth)
             return dumps({"statuscode": 0, "statusmsg": "success", "result": a1, "links": b1}, ensure_ascii=False)
         elif direction == "down":
             a2 = queryholders6(name, depth)
             b2 = queryholderss2(name, depth)
             return dumps({"statuscode": 0, "statusmsg": "success", "result": a2, "links": b2}, ensure_ascii=False)
         elif direction == "both":
